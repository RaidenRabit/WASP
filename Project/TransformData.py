...
def fillMissingWithNA(df): #replaces bad values with nan
...
def dropValues(df): #drops not needed columns
...
def replaceNA(df): #replaces or add names to deal with wrong data
...
def replaceValuesForEase(df): #replaces values so they are easyer to understand and use
...
def fixIncidentDate(df): #fixes INCIDENT_DATE based on given info
...
########################################## SORT DF ####################################
def sortDf(df): #Sorts values based on date and sets INCIDENT_DATE as index
...
def main():
    ...
    df=readFiles()
    df=fillMissingWithNA(df)
    df=replaceNA(df)
    df=convertObjToType(df)
    df=dropValues(df)
    df=replaceValuesForEase(df)
    df=fixIncidentDate(df)
    df=sortDf(df)
